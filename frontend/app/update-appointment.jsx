...
export default function UpdateAppointmentScreen() {
  ...
  const APPOINTMENT_STATUS_OPTIONS = (t) => [
    { label: t("scheduleStatus"), value: "scheduled" },
    { label: t("completeStatus"), value: "completed" },
    { label: t("cancelStatus"), value: "canceled" },
    { label: t("noShowStatus"), value: "no_show" },
  ];

  const [statusOpen, setStatusOpen] = useState(false);
  const statusItems = APPOINTMENT_STATUS_OPTIONS(t); // Get translated options
  ...
  // true if we have the data
  const isUpdate = !!initialAppointment;
  ...
  const validationSchema = Yup.object({
    ...
    startDateTime: Yup.date()
      ...
      .test(
        "is-future-on-create",
        t("appointmentStartDateTimePast"),
        function (value) {
          // If we are updating an existing record, skip this specific check.
          if (isUpdate) {
            return true;
          }
          // If we are creating, the date must be greater than the current date.
    ...
    function (values) {
      ...
      // Ensure that only one client type is provided.
        ...
        >
          <Formik
            ...
            initialValues={{
              ...
              userId: initialAppointment?.userId || null,
              external_client: initialAppointment?.external_client || null,
              startDateTime: initialAppointment?.startDateTime
                ? new Date(initialAppointment.startDateTime)
                : new Date(),
              endDateTime: initialAppointment?.endDateTime
                ? new Date(initialAppointment.endDateTime)
                : null,
            ...
            onSubmit={async (values, { setSubmitting }) => {
              ...
              try {
                if (payload.external_client) {
                  payload.userId = null;
                } else if (payload.userId) {
                  payload.external_client = null;
                }
                setSubmitting(true);
            ...
            {({
              ...
              <View style={styles.inputContainer}>
                <View style={styles.labelContainer}>
                  ...
                  />
                  <AppText style={styles.labelText}>Status</AppText>
                  <DropDownPicker
                    open={statusOpen}
                    value={values.status}
                    items={statusItems}
                    ...
                    listMode="SCROLLVIEW"
                    setValue={(callback) => {
                      const newValue = callback(values.status);
